...
class Date implements Interfaces\Date
{
     ...
     */
    public static function date_interval_second($interval, $basetime = 0)
    {
        if (Vars::is_decimal($interval)) {
            return $interval + 0;
        }
        ...
        if (!$interval instanceof \DateInterval) {
            ...
            $interval = Strings::preg_splice('#(\.\d+)S#', 'S', $interval, $m);
        ...
        $datetime = Date::date_convert(\DateTimeImmutable::class, $basetime);
        $difftime = $datetime->add($interval);

        return Date::date_timestamp($difftime) - Date::date_timestamp($datetime);
    }
