...
class Syntax implements Interfaces\Syntax
{
    ...
    {
        if (Utility::function_configure('chain.version') === 2) {
            return new class($source) implements \Countable, \ArrayAccess, \IteratorAggregate, \JsonSerializable {
                ...
                private static function _resolve($name, $data)
                {
                    ...
                    $isstringable = Vars::is_stringable($data);
                    if (false
                        // for global
                ...
                private static function _apply($callback, $arguments, $injections)
                {
                    ...
                    $metadata = self::$metadata[$callback] ??= (function ($callback) {
                        $reffunc = Funchand::reflect_callable($callback);
                    ...
                    foreach ($injections as $position => $injection) {
                        ...
                        // null セーフモード
                        if ($injection === null && Utility::function_configure('chain.nullsafe') && !($metadata['nullable'][$position] ?? false)) {
                            return null;
                        }
                    }
