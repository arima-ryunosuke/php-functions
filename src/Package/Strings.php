...
class Strings implements Interfaces\Strings
{
     ...
     */
    public static function strtr_escaped($string, $replace_pairs, $escape = '\\')
    {
        uksort($replace_pairs, fn($a, $b) => strlen($b) - strlen($a));
        $froms = array_keys($replace_pairs);

        $offset = 0;
        while (($pos = Strings::strpos_escaped($string, $froms, $offset, $escape, $found)) !== false) {
     ...
     */
    public static function glob2regex($pattern, $flags = 0)
    {
        $replacer = [
        ...
        $pattern = Strings::strtr_escaped($pattern, $replacer);

        if ($flags & GLOB_BRACE) {
            while (true) {
                $brace_s = Strings::strpos_escaped($pattern, '{');
                if ($brace_s === false) {
                    break;
                }
                $brace_e = Strings::strpos_escaped($pattern, '}', $brace_s);
                if ($brace_e === false) {
                    break;
                }
                $brace = substr($pattern, $brace_s + 1, $brace_e - $brace_s - 1);
                $brace = Strings::strtr_escaped($brace, [',' => '|']);
