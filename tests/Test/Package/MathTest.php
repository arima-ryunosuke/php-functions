...
class MathTest extends AbstractTestCase
{
    ...
    function test_mean()
    {
        ...
        that(mean)($data['datetime_odd'])->wasThrown('must be contain');
        that((mean)('1', 2, 3.5, 5.5, 'x'))->is(3.0);
    }

    function test_average()
    {
        that(average)()->wasThrown('not implement yet');
    }
    ...
    function test_sum()
    {
        ...
        that(sum)($data['string_evn'])->wasThrown('must be contain');
        that(sum)($data['string_odd'])->wasThrown('must be contain');
        that(sum)($data['datetime_evn'])->wasThrown('must be contain');
        that(sum)($data['datetime_odd'])->wasThrown('must be contain');
    ...
    function test_probability()
    {
        ...
        // 負数は NG
        that(probability)(-1, 1)->wasThrown('probability must be positive number');
        that(probability)(1, -1)->wasThrown('divisor must be positive number');
    }
